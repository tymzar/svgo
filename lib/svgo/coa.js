...
const { loadConfig, optimize, validate } = require('../svgo-node.js');
...
module.exports = function makeProgram(program) {
  program
    ...
    .option('--validate <VALIDATE...>', 'Input SVG file to validate')
    .option('--asset-type <TYPE...>', 'Input asset type')
    .option(
      '-o, --output <OUTPUT...>',
...
async function action(args, opts, command) {
  ...
  if (
    opts.validate != null &&
    opts.assetType &&
    Object.keys(opts.assetType).length == 1
  ) {
    var svgToVal = (output && output[0]) || opts.validate;

    await Promise.all([
      svgToVal.forEach((file, n) => {
        validateFile(config, file, svgToVal[n], opts.assetType[0]);
      }),
    ]);

    return;
  } else if (Object.keys(opts.assetType).length != 1) {
    console.error("error: option '--asset-type' takes only one param");
    process.exit(1);
  }
...
/**
 * Read SVG file and pass to validating process.
 * @param {Object} config options
 * @param {string} file
 * @param {string} output
 * @return {Promise}
 */
function validateFile(config, file, output, type) {
  return FS.promises.readFile(file, 'utf8').then(
    (data) => {
    ...
    (error) => checkValidateFileError(config, file, output, error)
  );
}
 ...
 * @return {Promise}
 */
function validateSVGData(config, info, data, output, input, fileName, type) {
  var startTime = Date.now(),
    prevFileSize = Buffer.byteLength(data, 'utf8');

  const [result] = validate(data, fileName, type, {
    ...config,
    ...info,
  });
 ...
 * @return {Promise}
 */
function checkValidateFileError(config, input, output, error) {
