...
const invokeValidatePlugins = require('./svgo/pluginsValidate.js');
...
const validate = (input, filename, type, config) => {
  ...
  for (let i = 0; i < maxPassCount; i += 1) {
    ...
    const validationAssetTypeConfig = validationAssetType[type];
    config = validationAssetTypeConfig.config;

    var validateResult = [];

    validateResult = validationAssetTypeConfig.resultTemplate; //add properties to svgjs
    svgjs.filename = filename;
    svgjs.type = type;
    ...
    [svgjs, validateResult] = invokeValidatePlugins(
      svgjs,
      info,
      resolvedPlugins,
      validateResult
